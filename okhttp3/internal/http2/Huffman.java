...
class Huffman
{
    ...
    private void addCode(int i, int n, byte b) {
        ...
        int n2;
        for (n2 = 8 - b, n = (i = (n << n2 & 0xFF)); i < (1 << n2) + n; ++i) {
    ...
    byte[] decode(final byte[] array) {
        ...
        while (j > 0) {
            final Node node2 = root.children[n2 << 8 - j & 0xFF];
            if (node2.children != null) {
                break;
            }
            if (node2.terminalBits > j) {
                break;
            }
    ...
    int encodedLength(final ByteString byteString) {
        long n = 0L;
        long n2;
        for (int i = 0; i < byteString.size(); ++i, n += n2) {
            n2 = Huffman.CODE_LENGTHS[byteString.getByte(i) & 0xFF];
        }
        return (int)(n + 7L >> 3);
    }
