...
public final class MixedItemSection extends Section
{
    ...
    static {
        TYPE_SORTER = new Comparator<OffsettedItem>() {
            @Override
            public int compare(final OffsettedItem offsettedItem, final OffsettedItem offsettedItem2) {
    ...
    public MixedItemSection(final String s, final DexFile dexFile, final int n, final SortType sort) {
        super(s, dexFile, n);
    ...
    public void add(final OffsettedItem offsettedItem) {
    ...
    public <T extends OffsettedItem> T get(final T t) {
        this.throwIfNotPrepared();
        final OffsettedItem offsettedItem = this.interns.get(t);
    ...
    public <T extends OffsettedItem> T intern(final T t) {
        synchronized (this) {
            this.throwIfPrepared();
            final OffsettedItem offsettedItem = this.interns.get(t);
            if (offsettedItem != null) {
    ...
    public void placeItems() {
        ...
        int i = 0;
        while (i < size) {
            final OffsettedItem offsettedItem = this.items.get(i);
            try {
                final int place = offsettedItem.place(this, writeSize);
                if (place < writeSize) {
                    final StringBuilder sb = new StringBuilder();
                    sb.append("bogus place() result for ");
                    sb.append(offsettedItem);
                    throw new RuntimeException(sb.toString());
                }
                writeSize = place + offsettedItem.writeSize();
                ++i;
                continue;
            }
    ...
    public void writeHeaderPart(final AnnotatedOutput annotatedOutput) {
        ...
        final char[] array = new char[15 - name.length()];
        Arrays.fill(array, ' ');
    ...
    public void writeIndexAnnotation(final AnnotatedOutput annotatedOutput, final ItemType itemType, String s) {
        this.throwIfNotPrepared();
        final TreeMap<String, OffsettedItem> treeMap = new TreeMap<String, OffsettedItem>();
        ...
        annotatedOutput.annotate(0, s);
        for (final Map.Entry<String, OffsettedItem> entry : treeMap.entrySet()) {
            s = entry.getKey();
            final OffsettedItem offsettedItem2 = entry.getValue();
            final StringBuilder sb = new StringBuilder();
            sb.append(offsettedItem2.offsetString());
            sb.append(' ');
            sb.append(s);
            sb.append('\n');
    ...
    @Override
    protected void writeTo0(final AnnotatedOutput annotatedOutput) {
        ...
        for (final OffsettedItem offsettedItem : this.items) {
            ...
            final int n4 = offsettedItem.getAlignment() - 1;
            final int n5 = n2 + n4 & ~n4;
            int n6;
            if ((n6 = n2) != n5) {
                annotatedOutput.writeZeroes(n5 - n2);
                n6 = n5;
            }
            offsettedItem.writeTo(file, annotatedOutput);
            n2 = n6 + offsettedItem.writeSize();
            n = n3;
        }
