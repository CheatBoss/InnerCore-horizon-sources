...
final class SortableType
{
    ...
    static {
        NULLS_LAST_ORDER = new Comparator<SortableType>() {
            @Override
            public int compare(final SortableType sortableType, final SortableType sortableType2) {
    ...
    public boolean tryAssignDepth(final SortableType[] array) {
        int n;
        if (this.classDef.getSupertypeIndex() == -1) {
            n = 0;
        }
        else {
            final SortableType sortableType = array[this.classDef.getSupertypeIndex()];
            if (sortableType == null) {
                n = 1;
            }
            else {
        ...
        final short[] interfaces = this.classDef.getInterfaces();
        for (int length = interfaces.length, i = 0; i < length; ++i) {
            final SortableType sortableType2 = array[interfaces[i]];
            if (sortableType2 == null) {
                n = Math.max(n, 1);
            }
            else {
                if (sortableType2.depth == -1) {
                    return false;
                }
                n = Math.max(n, sortableType2.depth);
            }
        }
        this.depth = n + 1;
        return true;
