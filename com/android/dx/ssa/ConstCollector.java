...
public class ConstCollector
{
    ...
    static {
        ConstCollector.COLLECT_STRINGS = false;
        ConstCollector.COLLECT_ONE_LOCAL = false;
    }
    
    private ConstCollector(final SsaMethod ssaMeth) {
        this.ssaMeth = ssaMeth;
    }
    
    private void fixLocalAssignment(final RegisterSpec registerSpec, RegisterSpec withLocalItem) {
        for (final SsaInsn ssaInsn : this.ssaMeth.getUseListForRegister(registerSpec.getReg())) {
            ...
            if (ssaInsn.getResult() == null) {
                continue;
            }
            final LocalItem localItem = localAssignment.getLocalItem();
            ssaInsn.setResultLocal(null);
            withLocalItem = withLocalItem.withLocalItem(localItem);
            final SsaInsn fromRop = SsaInsn.makeFromRop(new PlainInsn(Rops.opMarkLocal(withLocalItem), SourcePosition.NO_INFO, null, RegisterSpecList.make(withLocalItem)), ssaInsn.getBlock());
            final ArrayList<SsaInsn> insns = ssaInsn.getBlock().getInsns();
            insns.add(insns.indexOf(ssaInsn) + 1, fromRop);
        }
    }
    ...
    private ArrayList<TypedConstant> getConstsSortedByCountUse() {
        ...
        final HashSet<TypedConstant> set = new HashSet<TypedConstant>();
        for (int i = 0; i < regCount; ++i) {
            final SsaInsn definitionForRegister = this.ssaMeth.getDefinitionForRegister(i);
            if (definitionForRegister != null) {
                if (definitionForRegister.getOpcode() != null) {
                    final RegisterSpec result = definitionForRegister.getResult();
                    final TypeBearer typeBearer = result.getTypeBearer();
                    if (typeBearer.isConstant()) {
                        final TypedConstant typedConstant = (TypedConstant)typeBearer;
                        SsaInsn ssaInsn = definitionForRegister;
                        if (definitionForRegister.getOpcode().getOpcode() == 56) {
                            final ArrayList<SsaInsn> insns = this.ssaMeth.getBlocks().get(definitionForRegister.getBlock().getPredecessors().nextSetBit(0)).getInsns();
                            ssaInsn = insns.get(insns.size() - 1);
                        }
                        if (ssaInsn.canThrow()) {
                            if (!(typedConstant instanceof CstString)) {
                                continue;
                            }
                            if (!ConstCollector.COLLECT_STRINGS) {
                                continue;
                            }
                            if (ssaInsn.getBlock().getSuccessors().cardinality() > 1) {
                                continue;
                            }
                        }
                        if (this.ssaMeth.isRegALocal(result)) {
                            ...
                            if (set.contains(typedConstant)) {
                                continue;
                            }
                            set.add(typedConstant);
                        }
                        final Integer n = hashMap.get(typedConstant);
                        if (n == null) {
                            hashMap.put(typedConstant, 1);
                        }
                        else {
                            hashMap.put(typedConstant, n + 1);
                        }
                    }
                }
            }
        }
        final ArrayList<Object> list = (ArrayList<Object>)new ArrayList<TypedConstant>();
        for (final Map.Entry<TypedConstant, Integer> entry : hashMap.entrySet()) {
            if (entry.getValue() > 1) {
                list.add(entry.getKey());
            }
        }
        Collections.sort((List<K>)list, (Comparator<? super K>)new Comparator<Constant>() {
            @Override
            public int compare(final Constant constant, final Constant constant2) {
                int compareTo;
                if ((compareTo = hashMap.get(constant2) - hashMap.get(constant)) == 0) {
                    compareTo = constant.compareTo(constant2);
                }
                return compareTo;
            }
        ...
        return (ArrayList<TypedConstant>)list;
    }
    ...
    private void run() {
        final int regCount = this.ssaMeth.getRegCount();
        final ArrayList<TypedConstant> constsSortedByCountUse = this.getConstsSortedByCountUse();
        final int min = Math.min(constsSortedByCountUse.size(), 5);
        final SsaBasicBlock entryBlock = this.ssaMeth.getEntryBlock();
        final HashMap hashMap = new HashMap<TypedConstant, RegisterSpec>(min);
        for (int i = 0; i < min; ++i) {
            final TypedConstant typedConstant = constsSortedByCountUse.get(i);
            final RegisterSpec make = RegisterSpec.make(this.ssaMeth.makeNewSsaReg(), typedConstant);
            final Rop opConst = Rops.opConst(typedConstant);
            if (opConst.getBranchingness() == 1) {
                ...
                final SsaBasicBlock entryBlock2 = this.ssaMeth.getEntryBlock();
                final SsaBasicBlock primarySuccessor = entryBlock2.getPrimarySuccessor();
                final SsaBasicBlock insertNewSuccessor = entryBlock2.insertNewSuccessor(primarySuccessor);
        ...
        this.updateConstUses((HashMap<TypedConstant, RegisterSpec>)hashMap, regCount);
    }
    
    private void updateConstUses(final HashMap<TypedConstant, RegisterSpec> hashMap, final int n) {
        final HashSet<TypedConstant> set = new HashSet<TypedConstant>();
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (definitionForRegister != null) {
                final RegisterSpec result = definitionForRegister.getResult();
                final TypeBearer typeBearer = definitionForRegister.getResult().getTypeBearer();
                if (typeBearer.isConstant()) {
                    final TypedConstant typedConstant = (TypedConstant)typeBearer;
                    final RegisterSpec registerSpec = hashMap.get(typedConstant);
                    if (registerSpec != null) {
                        if (this.ssaMeth.isRegALocal(result)) {
                            ...
                            set.add(typedConstant);
                            this.fixLocalAssignment(result, hashMap.get(typedConstant));
                        }
                        ...
                        for (final SsaInsn ssaInsn : useListCopy[result.getReg()]) {
