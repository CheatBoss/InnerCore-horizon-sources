...
public class DualECSP800DRBG implements SP80090DRBG
{
    ...
    public DualECSP800DRBG(final DualECPoints[] array, final Digest digest, final int securityStrength, final EntropySource entropySource, final byte[] array2, final byte[] array3) {
        ...
        if (this._P != null) {
            final byte[] hash_df = Utils.hash_df(this._digest, concatenate, this._seedlen);
            this._s = hash_df;
    ...
    private byte[] pad8(final byte[] array, int n) {
        n %= 8;
        if (n == 0) {
            return array;
        }
        ...
        n = 0;
        int n3;
        for (int i = array.length - 1; i >= 0; --i, n = n3) {
            n3 = (array[i] & 0xFF);
            array[i] = (byte)(n >> 8 - n2 | n3 << n2);
        }
        return array;
    }
    ...
    public int generate(final byte[] array, byte[] array2, final boolean b) {
        final int length = array.length;
        final int n = array.length / this._outlen;
        if (Utils.isTooLarge(array2, 512)) {
        ...
        if (array3 != null) {
            array2 = Utils.hash_df(this._digest, array3, this._seedlen);
            scalarMultipleXCoord = new BigInteger(1, this.xor(this._s, array2));
        }
        else {
            scalarMultipleXCoord = new BigInteger(1, this._s);
        }
        int n2 = 0;
        ...
        int n3 = 0;
        while (i < n) {
            scalarMultipleXCoord = this.getScalarMultipleXCoord(this._P, scalarMultipleXCoord);
            final byte[] byteArray = this.getScalarMultipleXCoord(this._Q, scalarMultipleXCoord).toByteArray();
            final int length2 = byteArray.length;
            final int outlen = this._outlen;
            if (length2 > outlen) {
                System.arraycopy(byteArray, byteArray.length - outlen, array, n3, outlen);
            }
            else {
                System.arraycopy(byteArray, 0, array, outlen - byteArray.length + n3, byteArray.length);
            }
            n3 += this._outlen;
            ++this._reseedCounter;
            ++i;
        }
        BigInteger scalarMultipleXCoord2 = scalarMultipleXCoord;
        if (n3 < array.length) {
            scalarMultipleXCoord2 = this.getScalarMultipleXCoord(this._P, scalarMultipleXCoord);
            array2 = this.getScalarMultipleXCoord(this._Q, scalarMultipleXCoord2).toByteArray();
            final int length3 = array.length;
            final int length4 = array2.length;
            final int outlen2 = this._outlen;
            int n4;
            if (length4 > outlen2) {
                n2 = array2.length - outlen2;
                n4 = n3;
            }
            else {
                n4 = n3 + (outlen2 - array2.length);
            }
            System.arraycopy(array2, n2, array, n4, length3 - n3);
            ++this._reseedCounter;
        }
        this._s = BigIntegers.asUnsignedByteArray(this._sLength, this.getScalarMultipleXCoord(this._P, scalarMultipleXCoord2));
        return length * 8;
    }
    ...
    @Override
    public void reseed(byte[] concatenate) {
        if (!Utils.isTooLarge(concatenate, 512)) {
            concatenate = Arrays.concatenate(this.pad8(this._s, this._seedlen), this.getEntropy(), concatenate);
