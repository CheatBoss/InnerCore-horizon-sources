...
abstract class Icode
{
    ...
    static String bytecodeName(final int n) {
        if (!validBytecode(n)) {
    ...
    static boolean validBytecode(final int n) {
        return validIcode(n) || validTokenCode(n);
    }
    
    static boolean validIcode(final int n) {
        return -64 <= n && n <= 0;
    }
    
    static boolean validTokenCode(final int n) {
        return 2 <= n && n <= 80;
    }
}
